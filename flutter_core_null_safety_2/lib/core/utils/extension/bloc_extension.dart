...
extension LaunchRequestExtension on Bloc {
  ...
  @deprecated
  Stream<CoreState> launch<T>(
      Future<T> request, LoadingState loading, ResultState<T> resultState, ResultState<String> errorState) async* {
  ...
  Stream<CoreState> launchWithError<T, V>(
      Future<T> request, LoadingState loading, ResultState<T> resultState, ResultState<V> errorState) async* {
    yield loading.call(true);
    try {
      final result = await request;
      yield loading.call(false);
